...
ggplot(EU, aes(x=Year, y=Prop, colour=Taxa)) +
  geom_line()
  ggsave("figures/explore/Proportion of global from the EU incl Norway.tiff", device = "tiff", dpi=300, width = 15, height = 12, units = "cm")  #given EU regulations on by-products include Norway - no point using by products in Salmon as EU/Norway accounts for 60% of global production and a ban was in place
...
forage_TL <- read_csv("data/int/Trophic_level_2020.csv") |>  rename(FMFO_TL=TL)
forage_inclusion <- fread("data/inputs/forage_fish_diets.csv", header=TRUE)
...
(animal_parts <- 
    fread("data/inputs/diets_pahlow.csv", header = TRUE) |>  
    group_by(Taxa) |>  
...
#Have to temporarily do manually for sealife base species due to this issue https://github.com/ropensci/rfishbase/issues/244 in rfishbase (27th April 2023) - this maybe remedied in future runs
(shrimps <- tibble(Species = c("Litopenaeus vannamei", "Fenneropenaeus chinensis", "Fenneropenaeus indicus", "Penaeus monodon", "Marsupenaeus japonicus"), ComName = c("Whiteleg shrimp", "Fleshy prawn", "Banana prawn", "Giant tiger prawn", "Kuruma prawn"), Language = "English", SpecCode = NA))

(fw_crusts <- tibble(Species = c("Eriocheir sinensis", "Procambarus clarkii", "Macrobrachium nipponense", "Macrobrachium rosenbergii") , ComName = c("Chinese mitten crab", "Red swamp crawfish", "Oriental river prawn", "Giant river prawn"), Language = "English", SpecCode = ecology(c("Chinese mitten crab", "Red swamp crawfish", "Oriental river prawn", "Giant river prawn"))))
    
#shrimps <- rfishbase::common_to_sci(x = c("Whiteleg shrimp", "Fleshy prawn", "Banana prawn", "Giant tiger prawn", "Kuruma prawn"),                                server = "sealifebase") |>  slice(1,2,3,6,8))
#(fw_crusts <- common_to_sci(c("Chinese mitten crab", "Red swamp crawfish", "Oriental river prawn", "Giant river prawn"),  server = "sealifebase") |>  drop_na(Species))



# Trophic levels from sealife base seem to be encountering errors - so the plot cannot be run at present. Calculated of farm TLs can occur, however. 
...
#          mutate(Taxa = "Shrimps")),
# 
# rfishbase::ecology(shrimps$Species, server = "sealifebase")
# 
...
tl_export <- trophic_levels |>  
  select(Taxa, Year, Prod, FMFO_inclusion, Animal_inclusion, Crop_inclusion, TL_taxon_year) |>  
  write_csv("taxon_TLs.csv")


TL_FCR_plot <- 
  ggplot(trophic_levels |>  filter(Year %in% c(1995, 2000, 2010)) |>  mutate(Year= factor(Year, levels = (c("1995", "2000",  "2010")))), 
