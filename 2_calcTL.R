...
library(rfishbase)
library(ggplot2)
source('src/weighted_mean.R') # Calculates a weighted mean with NA in weights
# Get the catches of forage fish 

# df.old <- read.csv('marinecatches.csv')
# df <- df[df$size == 'Small',]

df.new <- read.csv('data/inputs/fao_catches_2019.csv')
...
# write.csv(df.export, file = 'data/int/TL_forage.csv', row.names = FALSE)
TL <- read.csv('data/int/TL_forage.csv')  
...
write.csv(file = 'data/int/Trophic_level_2020.csv', TL_new, row.names = FALSE)
