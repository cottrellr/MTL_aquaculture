...
MTL_sensitivity <- 
  ...
  scale_color_manual(values = c(rev(lacroix_palette("Lemon", n=10))[c(1,3,5,10)]), name = bquote(Allow~Delta~to))+
...
(salmon_diet <- data_frame(Year=rep(c("1990", "2000", "2016"), each=6, times=1),
  ...
  mutate(Group = case_when(Ingredient %in% c("Marine protein", "Marine oils") ~ "Fish",
                           Ingredient  == "Microingredients" ~ "Microingred.",
                           TRUE ~ "Plants"),
         Group = factor(Group, levels = c("Microingred.", "Plants", "Fish")))
  )                            
...
salmon_diet_plot <- ggplot(salmon_diet, aes(x=Year, y = Inclusion, fill = Group))+
  ...
  guides(fill=FALSE)+
  scale_fill_manual(values = lacroix_palette("PeachPear", n=3))+
  scale_y_continuous(expand = c(0,0), limits = c(0, 100))+
  annotation_custom(anchovy, xmin = c(0.7), xmax = c(1.3), ymin = c(44), ymax=c(50))+
  annotation_custom(anchovy,  xmin = c(1.8), xmax = c(2.2), ymin = c(26), ymax=c(31))+
  annotation_custom(anchovy, xmin = c(2.85), xmax = c(3.15), ymin = c(12), ymax=c(16))+
  annotation_custom(soy,  xmin = c(0.85), xmax = c(1.15), ymin = c(91), ymax=c(98))+
  annotation_custom(soy,  xmin = c(1.85), xmax = c(2.15), ymin = c(67), ymax=c(93))+
  annotation_custom(soy, xmin = c(2.8), xmax = c(3.2), ymin = c(30), ymax=c(90))+
...
ggplot(trophic_levels %>% filter(Year %in% c(1995, 2000, 2010)) %>% mutate(Year= factor(Year, levels = (c("1995", "2000",  "2010")))), 
       aes(x=TL_taxon_year, y=eFCR, colour=NA))+
  geom_mark_ellipse(aes(fill=Year, con.colour = NA))+
  ...
  theme(legend.position = "right",
        ...
        legend.title = element_blank())+
  scale_fill_manual(values = lacroix_palette("Pure", n=4))+
  scale_colour_manual(values = lacroix_palette("Pure", n=4))+
  labs(y="Feed conversion ratio", x="Taxon trophic level")+
  ggsave("Figure - FCR vs TL.tiff", device = "tiff", dpi=600, width = 11, height = 7, units = "cm")+
  ggsave("Figure - FCR vs TL.pdf", device = "pdf", dpi=600, width = 11, height = 7, units = "cm")
...
```
  ...
  #          Feeds_total = map(data, ~(gam(Feeds_used~s(Year), data = .))))
  # )
