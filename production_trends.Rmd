...
fed_spp <- 
  read_csv(here("data", "fed_spp.csv")) %>%
...
(fed_aqua_prod <- 
  read_csv(here("data", "aqua-prod-raw.csv")) %>% 
  rename(Country="Country (Country)",
         Species="Species (ASFIS species)" ,
         Area="Aquaculture area (FAO major fishing area)",
         Environment="Environment (Environment)")%>% 
...
ggplot(EU, aes(x=Year, y=Prop, colour=Taxa)) +
  ...
  theme_pubr()+
  ggsave(here("Proportion of global from the EU incl Norway.tiff"), device = "tiff", dpi=300, width = 15, height = 12, units = "cm") +
  ggsave(here("Proportion of global from the EU incl Norway.jpg"), device = "jpg", dpi=300, width = 15, height = 12, units = "cm") #given EU regulations on by-products include Norway - no point using by products in Salmon as EU/Norway accounts for 60% of global production and a ban was in place
...
forage_TL <- read_csv(here("data", "Trophic_level_2020.csv")) %>% rename(FMFO_TL=TL)
forage_inclusion <- fread(here("data", "forage_fish_diets.csv"), header=TRUE)
...
(animal_parts <- 
    fread(here("data", "diets_pahlow.csv"), header = TRUE) %>% 
    group_by(Taxa) %>% 
...
(by_products_plot <- ggplot(data = by_products %>% mutate(Animal_inclusion = case_when(Taxa=="Salmons" ~ 0, 
  ...
  theme_pubr()+
  ggsave(here("By product inclusion.tiff"), device = "tiff", dpi=300, width = 15, height = 12, units = "cm") +
  ggsave(here("By product inclusion.jpg"), device = "jpg", dpi=300, width = 15, height = 12, units = "cm"))
...
(forage_fish_TL <- ggplot(data = forage_TL, aes(x=Year, y=FMFO_TL))+
  ...
  theme_pubr()+
  ggsave(here("Forage TL.tiff"), device = "tiff", dpi=300, width = 15, height = 12, units = "cm") +
  ggsave(here("Forage TL.jpg"), device = "jpg", dpi=300, width = 15, height = 12, units = "cm"))
...
total_prod_year <- 
  read_csv(here("data", "forage_fish_diets.csv")) %>% 
  mutate(Prod=Prod*1000) %>% 
  group_by(Year) %>% 
  summarise(Total_prod = sum(Prod))

total_prod_year_2 <- 
  read_csv(here("data", "forage_fish_diets_2.csv")) %>% 
  mutate(Prod=Prod*1000) %>% 
  group_by(Year) %>% 
  summarise(Total_prod = sum(Prod))

total_prod_year_3 <- 
  read_csv(here("data", "forage_fish_diets_3.csv")) %>% 
  mutate(Prod=Prod*1000) %>% 
  group_by(Year) %>% 
  summarise(Total_prod = sum(Prod))
...
total_prod_year_4 <- 
  read_csv(here("data", "forage_fish_diets_4.csv")) %>% 
  mutate(Prod=Prod*1000) %>% 
  group_by(Year) %>% 
  summarise(Total_prod = sum(Prod))
...
aqua_trophic_levels <- 
  bind_rows(
  (TL_delta_global_full <- fread(here("data", "forage_fish_diets.csv"), header=TRUE) %>% 
     filter(Year!=2020) %>% 
  ...
  (TL_delta_global_FFTL_delta <- fread(here("data","forage_fish_diets_2.csv"), header=TRUE) %>% 
      filter(Year!=2020) %>% 
      left_join(by_products, by=c("Taxa", "Year")) %>%
      mutate(FMFO_inclusion=Total/100,
            Animal_inclusion = case_when(Taxa == "Salmons" ~ 0,
  ...
  (TL_delta_global_Incl_delta <- fread(here("data","forage_fish_diets_3.csv"), header=TRUE) %>% 
      filter(Year!=2020) %>% 
      left_join(by_products, by=c("Taxa", "Year")) %>%
      mutate(FMFO_inclusion=Total/100,
             Animal_inclusion = case_when(Taxa == "Salmons" ~ 0,
  ...
  (TL_delta_global_Incl_delta <- fread(here("data","forage_fish_diets_4.csv"), header=TRUE) %>% 
      filter(Year!=2020) %>% 
      left_join(by_products, by=c("Taxa", "Year")) %>%
      mutate(FMFO_inclusion=Total/100,
             Animal_inclusion = case_when(Taxa == "Salmons" ~ 0,
...
<<<<<<< HEAD
  ...
  write_csv("sector_trophic_levels_sensitivity.csv")
=======
  )


#Repeat the all variables but with linear anima-byproduct inclusion

bind_rows(
  (exponential <- fread(here("data", "forage_fish_diets.csv"), header=TRUE) %>% 
     filter(Year!=2020) %>% 
     left_join(by_products, by=c("Taxa", "Year")) %>%
     mutate(FMFO_inclusion=Total/100,
            Animal_inclusion = case_when(Taxa == "Salmons" ~ 0,
  ...
  (linear <- fread(here("data", "forage_fish_diets.csv"), header=TRUE) %>% 
    filter(Year!=2020) %>% 
    left_join(by_products, by=c("Taxa", "Year")) %>%
    mutate(FMFO_inclusion=Total/100,
   ...
   (sal_byp <- fread(here("data", "forage_fish_diets.csv"), header=TRUE) %>% 
     filter(Year!=2020) %>% 
    left_join(by_products, by=c("Taxa", "Year")) %>%
    mutate(FMFO_inclusion=Total/100,
           # Animal_inclusion_2 = case_when(Taxa == "Salmons" ~ 0,
  ...
  ggplot(aes(x=Year, y=MTL, colour=Sensitivity))+geom_line()+
  theme_pubr()+
  ...
  ggsave(here("Figure S2 - by product sensitivity.jpg"), device = "jpg", dpi=300, width=12, height = 10, units="cm")+
  ggsave(here("Figure S2 - by product sensitivity.pdf"), device = "pdf", dpi=300, width=12, height = 10, units="cm")
...
MTL_sensitivity+annotation_custom(grob = ggplotGrob(salmon_diet_plot), 
                                  xmin = 1994.5,
                                  xmax = 2006,
                                  ymin = 2.15,
                                  ymax = 2.38)+
   ggsave("Figure - diet and MTL sensitivity_2.tiff", device ="tiff", dpi=600, width = 15, height= 10, units = "cm")+
  ggsave("Figure - diet and MTL sensitivity_2.pdf", device ="pdf", dpi=600, width = 15, height= 10, units = "cm")+
    ggsave("Figure - diet and MTL sensitivity_2.jpg", device ="jpg", dpi=600, width = 15, height= 10, units = "cm")



class(aqua_trophic_levels$Year)
...
(salmons <- rfishbase::common_to_sci(c("Atlantic salmon", "Coho salmon", "Chinook salmon")) %>% slice(1,4,5))
...
(carp_wild <- ggplot(Fishbase_TLs %>% filter(Taxa =="Carps"))+
  geom_boxplot(aes(y=TL_final, middle=median(TL_final), lower=quantile(TL_final,0.95), upper=quantile(TL_final,0.95)), fill="transparent", colour="firebrick")+
...
scale_y_continuous(limits = c(2,3.1), expand = c(0,0))
...
(eel_wild <- ggplot(Fishbase_TLs %>% filter(Taxa =="Eels"))+
  ...
  theme(panel.background = element_rect(fill="transparent", colour = "transparent"),
        ...
        element_blank(),
        plot.margin = unit(c(0,0,0,0), "cm"))+
scale_y_continuous(limits = c(2,3.6), expand = c(0,0))
...
(milk_wild <- ggplot(Fishbase_TLs %>% filter(Taxa =="Milkfish"))+
  ...
  theme(panel.background = element_rect(fill="transparent", colour = "transparent"),
        ...
        element_blank(),
        plot.margin = unit(c(0,0,0,0), "cm"))+
scale_y_continuous(limits = c(2,2.5), expand = c(0,0))
...
TL_taxa <- fread(here("data", "forage_fish_diets.csv"), header=TRUE) %>% 
     filter(Year!=2020) %>% 
     left_join(by_products, by=c("Taxa", "Year")) %>%
     mutate(FMFO_inclusion=Total/100,
            Animal_inclusion = case_when(Taxa == "Salmons" ~ 0,
...
carp_plot <- ggplot(TL_taxa %>% filter(Taxa=="Carps"), aes(x=Year, y=TL_taxon_year))+geom_line()+
...
catfish_plot <- ggplot(TL_taxa %>% filter(Taxa=="Catfishes"), aes(x=Year, y=TL_taxon_year))+geom_line()+
...
eel_plot <- ggplot(TL_taxa %>% filter(Taxa=="Eels"), aes(x=Year, y=TL_taxon_year))+geom_line()+
...
crust_plot <- ggplot(TL_taxa %>% filter(Taxa=="FW crustaceans"), aes(x=Year, y=TL_taxon_year))+geom_line()+
...
fw_plot <- ggplot(TL_taxa %>% filter(Taxa=="FW fishes"), aes(x=Year, y=TL_taxon_year))+geom_line()+
  scale_y_continuous(limits=c(2,4.5))+
...
marine_plot <- ggplot(TL_taxa %>% filter(Taxa=="Marine fishes"), aes(x=Year, y=TL_taxon_year))+geom_line()+
  scale_y_continuous(limits=c(2,4.5))+
...
milk_plot <- ggplot(TL_taxa %>% filter(Taxa=="Milkfish"), aes(x=Year, y=TL_taxon_year))+geom_line()+
...
salm_plot <- ggplot(TL_taxa %>% filter(Taxa=="Salmons"), aes(x=Year, y=TL_taxon_year))+geom_line()+
  scale_y_continuous(limits=c(2,4.5))+
...
shrimp_plot <- ggplot(TL_taxa %>% filter(Taxa=="Shrimps"), aes(x=Year, y=TL_taxon_year))+geom_line()+
...
tilapia_plot <- ggplot(TL_taxa %>% filter(Taxa=="Tilapias"), aes(x=Year, y=TL_taxon_year))+geom_line()+
  scale_y_continuous(limits=c(2,2.5))+
...
trout_plot <- ggplot(TL_taxa %>% filter(Taxa=="Trouts"), aes(x=Year, y=TL_taxon_year))+geom_line()+
...
(legend_plot <- ggplot()+theme(rect = element_blank())+
  ...
  annotate("segment", x=0.1, xend = 0.3, y=0.4, yend=0.4, color="firebrick")+
    annotate("segment", x=0.1, xend = 0.3, y=0.75, yend=0.75, color="black")+
    annotate("rect", xmin = 0.1, xmax = 0.3, ymin = 0.3, ymax=0.5, fill="transparent", colour="firebrick")+
    annotate("text", y=0.75, x=0.4, label = "Farmed\neffective", size=2.8, hjust=0)+
...
annotate_figure(p = ggarrange(carp_plot, catfish_plot, eel_plot, crust_plot,
          ...
          left = text_grob("Mean trophic level", size=11, rot = 90),
          bottom = text_grob("Year", size=11))+
  ggsave("Fishbase_v_calculated_2.tiff", device = "tiff", dpi = 300, width=15, height = 18, units = "cm")+
   ggsave("Fishbase_v_calculated_2.jpg", device = "jpg", dpi = 300, width=15, height = 18, units = "cm")
...
TL_taxa %>%
  ...
  select(Taxa, Year, TL_taxon_year, TL_final) %>%
  group_by()
  
  
  rename(Farmed = TL_taxon_year,
         Wild = Fishbase) %>% 
...
mean(Fishbase_TLs %>% filter(Taxa=="Catfishes") %>% .$TL_final)
...
FCRs <- fread(here("data",  "eFCRs.csv"), header=TRUE)


(trophic_levels <- 
  fread(here("data", "forage_fish_diets.csv"), header=TRUE) %>% 
  filter(Year!=2020) %>% 
  left_join(by_products, by=c("Taxa", "Year")) %>%
  mutate(FMFO_inclusion=Total/100,
         Animal_inclusion = case_when(Taxa == "Salmons" ~ 0,
...
tl_export <- trophic_levels %>% 
  select(Taxa, Year, Prod, FMFO_inclusion, Animal_inclusion, Crop_inclusion, Total_prod, TL_taxon_year) %>% 
  write_csv("taxon_TLs.csv")
...
(TL_FCR_plot <- ggplot(trophic_levels %>% filter(Year %in% c(1995, 2005, 2015) & !Taxa %in%c("FW fishes", "Milkfish")) %>% mutate(Year= factor(Year, levels = (c("1995", "2005",  "2015")))), 
  ...
  geom_text(data = trophic_levels %>%
                  ...
                  force = 0.7,
                  point.padding = 0.5)+
    geom_segment(x= 2.132447, xend = 2.301294, y=2.0, yend=1.6, colour="black", linetype=3)+
    geom_segment(x=2.301294, xend = 2.131350, y=1.6, yend=1.4, colour="black", linetype=3)+
  ...
  labs(y="Feed conversion ratio", x="Trophic level"))+
    ...
    ggsave( "Figure - FCR vs TL.jpg", ggMarginal(TL_FCR_plot, groupColour = TRUE, groupFill = TRUE, xparams = list( size=1), yparams = list(size=1)), device = "jpg", dpi=600, width = 18, height = 13, units = "cm")



unique(trophic_levels$Taxa)
```
