...
ggplot(EU, aes(x=Year, y=Prop, colour=Taxa)) +
  geom_line()+
  ggsave("Proportion of global from the EU incl Norway.tiff", device = "tiff", dpi=300, width = 15, height = 12, units = "cm")
...
#Combine forage fish inclusion and model increase in animal byproduct through time with teh decrease in forage fish
...
(feed_change <- data_frame(Taxa = rep(unique(delta_forage_fish$Taxa), each=length(seq(1995,2015)), n=1),
    ...
    left_join(by_products, by=c("Taxa", "Year")) %>% 
    mutate(Rendered_animal_prop = case_when(Year %in% c(1995, 2015) & Taxa %in% c("Carps", "Eels") ~ 0.0001,
                                            Year ==1995 ~ 0.0005,
